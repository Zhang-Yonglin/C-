...
//规则4，  如果是私有(private)继承， 儿子中的父亲的除了private成员，其余在儿子中都是private成员。


class Parent
{
public:
	int pub = 1;    //在类的内部 和 外部 都能访问。
protected:
	int pro = 1;    //在类的内部可以访问， 在类的外部不可以访问
private:
	int pri = 1;    //在类的内部可以访问， 在类的外部不可以访问
};
...
public:
	void func2() 
	{
		pub;     //此时pub通过protected继承 能够在类的内部访问。 
			 ...
			 //pub 就是protected成员
		pro;     //pro 跟pub 是一样的性质，pro也是protected成员
		//pri;
	}
};
...
//在C++中 可重用性 是通过继承这一机制来实现的。
//可重用性即一段代码写完之后就不用动了，如果需要进行补充，则重用以前的代码，并加以补充新的代码
